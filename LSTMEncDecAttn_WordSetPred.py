...
def decodeByBeamFast(EncDecAtt, encSent, cMBSize, max_length, beam_size, args, reference_wordset, f_dGraph, REDmodel, tgt_wordSet_vec):
    train_mode = 0  # 評価なので
...
def ttest_model(args):
    ...
    with io.open(args.encDataFile, encoding='utf-8') as f, io.open(args.decDataFile, encoding='utf-8') as g, open("output_dGraph.txt", "w") as f_dGraph:
        ...
        for sentence, ref_sentence in zip(f, g):
            ...
            tgt_wordSet_vec = REDmodel.fwd(chainer.Variable(sourceSentence_vec)).data
            th = float(args.wordset_th)

            tgt_wordSet_vec[0][th <= tgt_wordSet_vec[0]] = 1.0
            tgt_wordSet_vec[0][th > tgt_wordSet_vec[0]] = 0
            ...
            pre = float(tp) / (tp + fp)
            rec = float(tp) / (tp + fn)
            pre_rec.write("@%@ origin sent {} pre {} rec {} len {} tp {} fp {} fn {}\n".format(counter, pre, rec, ln, tp, fp, fn))
            sys.stderr.write("@%@ origin sent {} pre {} rec {} len {} tp {} fp {} fn {}\n".format(counter, pre, rec, ln, tp, fp, fn))
            # word given
            gvnWord = args.nGivenWord
            w_counter = 0
            for w_index in list(reference_wordset):
                if w_counter >= gvnWord:
                    break
                if tgt_wordSet_vec[0][w_index] == 0:
                    tgt_wordSet_vec[0][w_index] = 1.0
                    w_counter += 1
            tgt_wordSet_vec[0] 
            tgt_wordSet_vec[0][th <= tgt_wordSet_vec[0]] = 1.0
            tgt_wordSet_vec[0][th > tgt_wordSet_vec[0]] = 0
            ...
            pre = float(tp) / (tp + fp)
            rec = float(tp) / (tp + fn)
            pre_rec.write("@%@ new sent {} pre {} rec {} len {} tp {} fp {} fn {}\n".format(counter, pre, rec, ln, tp, fp, fn))
            sys.stderr.write("@%@ new sent {} pre {} rec {} len {} tp {} fp {} fn {}\n".format(counter, pre, rec, ln, tp, fp, fn))
            ...
            outputBeam = decodeByBeamFast(EncDecAtt, sourceSentence, cMBSize,
                                          ...
                                          reference_wordset, f_dGraph, REDmodel,
                                          tgt_wordSet_vec)
            wposi = 4
            outloop = 1
