...
class RSChain(chainer.Chain):
    def __init__(self, inDim, hDim, outDim):
        super(RSChain, self).__init__(
            l1 = chaLink.Linear(inDim, hDim),
            l2 = chaLink.Linear(hDim, outDim), 
        )

    def __call__(self, x, y): 
        fv = self.fwd(x)
        loss = chaFunc.mean_squared_error(fv, y)
        return loss

    def fwd(self, x): 
        h1 = chaFunc.sigmoid(self.l1(x))
        h2 = self.l2(h1)
        return h2
...
class EncoderDecoderAttention:
    ...
    def trainOneMiniBatch(self, train_mode, decSent, encInfo,
        ...
        pred_mat = list() # 評価しない時のためにオブジェクトを作成しておく
        corr_sent_mat = list()
        for index in reversed(six.moves.range(decoder_proc)):
            ...
            if train_mode == 0 or args.doEvalAcc > 0:
                ...
                pred_mat.append(pred_arr) 
                corr_sent_mat.append(correctLabel)
                # 正解と予測が同じなら0になるはず
                # => 正解したところは0なので，全体から引く
        ...
        # リアルタイムでBLEU評価したいのでcorr_sent_matとpred_matをreturnに追加した
        return total_loss_val, (correct, incorrect, decoder_proc, proc, corr_sent_mat, pred_mat) 
...
def train_model_sub(train_mode, epoch, tData, EncDecAtt, optimizer,
    ...
    if 1:  # 並列処理のコードとインデントを揃えるため．．．
        ...
        # メインループ
        if train_mode == 0:
            with open("/home/takebayashi/src/LSTMEncDecAttn/dev_ref.txt", 'w') as fr:
                fr.write("") # 初期化 
            with open("/home/takebayashi/src/LSTMEncDecAttn/dev_hyp.txt", 'w') as fh:
                fh.write("") # 初期化 
        ...
        for encSent, decSent in tData:
            ...
            # try:
                ...
                hsent = list()
                if train_mode == 0: # dev
	            corr_sent_mat_t = np.array(acc_stat[4]).T
	            pred_mat_t = np.array(acc_stat[5]).T
                    with open("/home/takebayashi/src/LSTMEncDecAttn/dev_ref.txt", 'a') as fr:
                        for line in corr_sent_mat_t:
                            for w_idx in reversed(line):
                                if w_idx == 2:
                                    break
                                else:
                                    rsent.append(w_idx)
                            fr.write("{}\n".format(" ".join([EncDecAtt.index2decoderWord[int(idx)].encode('utf-8') for idx in rsent if idx != -1])))
                    with open("/home/takebayashi/src/LSTMEncDecAttn/dev_hyp.txt", 'a') as fh:
                        for line in pred_mat_t:
                            for w_idx in reversed(line):
                                if w_idx == 2:
                                    break
                                else:
                                    hsent.append(w_idx)
                            fh.write("{}\n".format(" ".join([EncDecAtt.index2decoderWord[int(idx)].encode('utf-8') for idx in hsent if idx != -1])))                
            ...
            if False: # try and except を無視するため
...
def train_model(args):
    ...
    # 学習のループ
    # トレーニング済モデルを途中から訓練する場合、epochが0から始まってしまう問題
    # init_epochを使って対応
    init_epoch = 0
    if args.setting_file and args.init_model_file:
        init_epoch = int("".join([init_w.replace("epoch", "") for init_w in args.init_model_file.split('.') if "epoch" in init_w]))
    for epoch in six.moves.range(init_epoch, args.epoch):
        ####################################
        # devの評価モード
        if args.encDevelDataFile and args.decDevelDataFile:
            train_mode = 0
            ...
            # BLEUを評価してここで表示させたい
            bleu_score = commands.getoutput('/home/takebayashi/src/Postprocess/multi-bleu.perl /home/takebayashi/src/LSTMEncDecAttn/dev_ref.txt < /home/takebayashi/src/LSTMEncDecAttn/dev_hyp.txt')
            sys.stdout.write('\r# Dev.Data | %s | diff: %e | %s\n' % (
                msgA, dL / max(1, tInfo.instanceNum), bleu_score))
...
def decodeByBeamFast(EncDecAtt, encSent, cMBSize, max_length, beam_size, args, reference_wordset, f_dGraph, tgt_wordSet_vec):
    train_mode = 0  # 評価なので
    ...
    beam = [(0, [idx_bos], idx_bos, encInfo.lstmVars, finalHS, WFilter, 0)]
    dummy_b = (1.0e+100, [idx_bos], idx_bos, None, None, WFilter, 0)
    ...
    for i in six.moves.range(max_length + 1):  # for </s>
        ...
        #sys.stderr.write("{}\n".format(nextWordProb_a[0]))
        for z, b in enumerate(beam):
            nextWordProb_a[z] -= args.red_weight * xp.array(tgt_wordSet_vec[0])
        #sys.stderr.write("{}\n".format(nextWordProb_a[0]))
        ...
        for z, b in enumerate(beam):
            ...
            if flag_force_eval:
                ...
                nb = (newProb, b[1][:] + [wordIndex], wordIndex,
                      lstm_states, next_h4, tWFilter, 0)
            ...
            for wordIndex in sortedIndex_a[z]:
                newProb = nextWordProb[wordIndex] + b[0]
                #sys.stderr.write("nextWordProb[wordIndex]{}\n".format(nextWordProb[wordIndex]))

                # FFNNで単語集合を予想
                # src_wordSet_vec = np.zeros((1, len(EncDecAtt.encoderVocab)), dtype=np.float32)
                ### src_wordSet_vec = np.zeros((1, 29055), dtype=np.float32)
                ### src_wordSet_vec[0][b[1] + [wordIndex]] = 1.0
                # tgt_wordSet_vec = REDmodel.fwd(chainer.Variable(src_wordSet_vec)).data
                #print("{} / {}".format([word for word in b[1]+[wordIndex]], len(b[1])))
                # diff = calc_diff.difference_set(reference_wordset, set(b[1] + [wordIndex]), margin=args.margin)
                ### diff = calc_diff.difference(tgt_wordSet_vec[0], src_wordSet_vec[0], margin=args.margin)
                ### score = newProb / (len(b[1]) + 1) + args.red_weight * diff
                score = newProb / (len(b[1]) + 1)
                diff = 0
                # score = newProb
                ## REDの計算
                ##### inputEmb = EncDecAtt.getDecoderInputEmbeddings(np.array([wordIndex], dtype=np.int32), args)
                ##### redVec = chaFunc.concat((next_h4, inputEmb))
                ##### redVec_tmp = redVec.data
                ##### REDScore = REDmodel.fwd(chainer.cuda.to_cpu(redVec_tmp))
                # 論文の式
                ##### len_now = len(b[1]) - 1 #bosのぶん
                ##### words = b[1][:] + [wordIndex]
                ##### score = newProb + float(args.red_weight) * abs(t_len - len_now - REDScore.data[0][0])
                if score > newBeam[-1][0]: #従来のprobとREDを考慮して比較するが、保存するのはprobのみ
                    continue
                    # break
                # ここまでたどり着いたら入れる
                if args.wo_rep_w:
                    ...
                    tWFilter = b[5]
                nb = (newProb, b[1][:] + [wordIndex], wordIndex,
                      lstm_states, next_h4, tWFilter, diff)
                newBeam = updateBeamThreshold__2(newBeam, nb)
                #####
        ################
        # 一時刻分の処理が終わったら，入れ替える
        ...
        for b in beam:
            f_dGraph.write("{}\n".format(' '.join(
                [EncDecAtt.index2decoderWord[word].encode('utf-8') for word in b[1]])))
            f_dGraph.write("C\t {}\nC/t\t {}\n".format(b[0], b[0] / (len(b[1]) - 1)))
            f_dGraph.write("Q\t {}\nQ'\t {}\n".format(b[6], b[6]))
...
def ttest_model(args):
    # tmp_vec = chainer.Variable(np.random.rand(2,1024).astype(np.float32))
    # tmp_scr = REDmodel.fwd(tmp_vec)
    # print(tmp_scr)
    EncDecAtt = pickle.load(open(args.setting_file, 'rb'))
    EncDecAtt.initModel()
    ...
    encVocabSize = len(EncDecAtt.encoderVocab)
    decVocabSize = len(EncDecAtt.decoderVocab)
    if args.transDir == 'ja-en':
        encVocabSize += 1
    # REDmodel = RSChain(len(EncDecAtt.encoderVocab), args.WSP_hDim, len(EncDecAtt.decoderVocab))
    # REDmodel = RSChain(encVocabSize, args.WSP_hDim, decVocabSize)
    # chaSerial.load_npz(args.REDmodel, REDmodel)
    ...
    begin = time.time()
    counter = 0
    pre_rec = open("./pre_rec.th{}.givenWord{}.{}.txt".format(args.wordset_th, args.nGivenWord, args.transDir), "w")
    # TODO: codecsでないとエラーが出る環境がある？ 要調査 不要ならioにしたい
    with io.open(args.encDataFile, encoding='utf-8') as f, io.open(args.decDataFile, encoding='utf-8') as g, open("output_dGraph.txt", "w") as f_dGraph:
        # with codecs.open(args.encDataFile, encoding='utf-8') as f:
        for sentence, ref_sentence in zip(f, g):
            sentence = sentence.strip()  # stripを忘れずに．．．
            ref_sentence = ref_sentence.strip()  # stripを忘れずに．．．
            ref_id_list = prepD.sentence2index(
                ref_sentence, EncDecAtt.decoderVocab, input_side=False)
            reference_wordset = set(ref_id_list)
            reference_wordset_vec = np.zeros(decVocabSize)
            reference_wordset_vec[list(reference_wordset)] = 1
            ...
            sourceSentence_vec = np.zeros((1, encVocabSize), dtype=np.float32)
            sourceSentence_vec[0][sourceSentence] = 1.0
            # referenceを用いた場合
            # tgt_wordSet_vec = np.array([reference_wordset_vec])

            edict_j2e = json.load(open(args.j2e_dict, 'r'))
            e_words_idx = [] 
            sys.stdout.write("\nsent number {}\n".format(counter))
            sys.stdout.write("  {}\n".format(sentence.encode('utf-8')))
            #print("sent number {}".format(counter))
            #print("  {}".format(sentence.encode('utf-8')))
            for src_w in sentence.split(" "):
                if src_w in edict_j2e:
                    e_words = edict_j2e[src_w]
                    e_words_idx.extend([EncDecAtt.decoderVocab[w] for w in e_words if w in EncDecAtt.decoderVocab])
            # 確認
            sys.stdout.write("  {}\n".format(" ".join([EncDecAtt.index2decoderWord[idx].encode('utf-8') for idx in e_words_idx])))
            #print("  {}".format(" ".join([EncDecAtt.index2decoderWord[idx].encode('utf-8') for idx in e_words_idx])))
            tgt_wordSet_vec = np.zeros((1, decVocabSize))
            tgt_wordSet_vec[0][e_words_idx] = 1

            ln = sum(tgt_wordSet_vec[0])
            tp = len([1 for h, r in zip(tgt_wordSet_vec[0], reference_wordset_vec) if h==1 and r==1])
            fp = len([1 for h, r in zip(tgt_wordSet_vec[0], reference_wordset_vec) if h==1 and r==0])
            fn = len([1 for h, r in zip(tgt_wordSet_vec[0], reference_wordset_vec) if h==0 and r==1]) 
            pre = float(tp) / (tp + fp) if (tp + fp) > 0 else -1
            rec = float(tp) / (tp + fn) if (tp + fn) > 0 else -1
            #print("  @%@ new sent {} pre {} rec {} len {} tp {} fp {} fn {}".format(counter, pre, rec, ln, tp, fp, fn))
            sys.stdout.write("  @%@ new sent {} pre {} rec {} len {} tp {} fp {} fn {}\n".format(counter, pre, rec, ln, tp, fp, fn))
             
            idx_eos = EncDecAtt.decoderVocab['</s>']
            tgt_wordSet_vec[0][idx_eos] = 1.0 # 終わらなくるから
            outputBeam = decodeByBeamFast(EncDecAtt, sourceSentence, cMBSize,
                                          decMaxLen, args.beam_size, args,
                                          reference_wordset, f_dGraph,
                                          tgt_wordSet_vec)
            wposi = 4
            outloop = 1
            ...
            for i in six.moves.range(outloop):
                ...
                sys.stdout.write('{}\n'.format(
                    ' '.join(word.encode('utf-8') for word in outputList[1:len(outputList) - 1])))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--red-model',
        dest='REDmodel',
        help=('name of red model'))
    parser.add_argument(
        '--red-weight',
        dest='red_weight',
        type=float,
        help=('weight of red: how much forcus on red score'))
    # file of target length
    parser.add_argument(
        '--target-len',
        dest='targetLenFile',
        default='',
        help='filename of target length')
    # margin for calculate difference 
    parser.add_argument(
        '--diff-margin',
        dest='margin',
        default='5',
        type=int,
        help='margin of word set pred')
    # for hidden dimension of WordSetPrediction model
    parser.add_argument(
        '--wsp-hidden-dim',
        dest='WSP_hDim',
        default='512',
        type=int,
        help='hidden dim of wsp model')
    # for hidden dimension of WordSetPrediction model
    parser.add_argument(
        '--n-given-word',
        dest='nGivenWord',
    ...
    parser.add_argument(
        ...
        default='',
        type=str,
        help='translate direction e.g)en-ja, ja-en')
    # for j2e_dict (edict) 
    parser.add_argument(
        '--j2e-dict',
        dest='j2e_dict',
