...
def decodeByBeamFast(EncDecAtt, encSent, decSent, cMBSize, max_length, beam_size, args):
    outputLineList = list()
    ...
    decSent = [w[0] for w in decSent]
    decSent.append(idx_eos) # force decoding するときは</s>を入れないと終わらない
    sys.stderr.write("decSent: {}\n".format(decSent))
    sys.stderr.write("decSent: {}\n".format([EncDecAtt.index2decoderWord[w] for w in decSent]))
    if args.wo_rep_w:
    ...
    for i in six.moves.range(max_length + 1):  # for </s>
        ...
        # bn.argpartitionがない
        # Force Decoding ではsortedIndex_aは使わないのでコメントアウト
        ...
        #######################################################################
        # 入力となる内部状態と出力である文の長さ finalHS
        partOfSent = ','.join([str(w_idx) for w_idx in newBeam[0][1]])
        outputLineList.append(','.join([str(elm) for elm in finalHS_a[0].data]) + '|' + str(len(newBeam[0][1])) + '|' + partOfSent + '\n')
        ################
        # 一時刻分の処理が終わったら，入れ替える
        beam = newBeam
...
def ttest_model(args):
    # with open('Data_unproc_RemainScore.txt', 'w') as f:
    #    f.write('') # 追加モードで開きたいのでここで初期化しておく 
    out_file = open('/media/takebayashi/064AD3034AD2EE85/RemainLength/Data.txt', 'w')
    EncDecAtt = pickle.load(open(args.setting_file, 'rb'))
    EncDecAtt.initModel()
    ...
    outputline_len = 0
    # TODO: codecsでないとエラーが出る環境がある？ 要調査 不要ならioにしたい
    with io.open(args.encDataFile, encoding='utf-8') as f, io.open(args.decDataFile, encoding='utf-8') as g:
        # with codecs.open(args.encDataFile, encoding='utf-8') as f:
        for sentence, dec_sentence in six.moves.zip(f, g):
            ...
            targetSentence = prepD.sentence2index(
                dec_sentence, EncDecAtt.decoderVocab, input_side=True) # <s>をつけてはいけないが</s>は必要
            targetSentence = np.transpose(
                np.reshape(np.array(targetSentence, dtype=np.int32),
                           (1, len(targetSentence))))
            # 1文ずつ処理するので，test時は基本必ずminibatch=1になる
            cMBSize = len(sourceSentence[0])
            outputBeam, outLineList = decodeByBeamFast(EncDecAtt, sourceSentence, targetSentence, cMBSize,
                                          decMaxLen, args.beam_size, args)
            outputline_len += len(outLineList)
            out_file.writelines(outLineList)
            if outputline_len > 1500000: #出力ファイルの長さが150万を超えたらおしまい
                    break
            wposi = 4
            outloop = 1
            ...
            for i in six.moves.range(outloop):
                ...
                # use .encode('utf-8') because codec error occur 
                sys.stdout.write('{}\n'.format(
